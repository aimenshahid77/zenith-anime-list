...
import { ChevronLeft, Heart, MessageSquare, Send, Star } from "lucide-react";

function MangaDetails() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { mangaDetails } = useMangaDetails(Number(id));
  if (mangaDetails.isPending) return <div>Loading...</div>;
  if (!mangaDetails.data) return null;
  const data = mangaDetails.data.data;
  console.log(data);
  return (
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  {data.genres.map((genre: any) => (
                    <span
                      key={genre.mal_id}
                      className="px-5 py-2 bg-blue-500/10 backdrop-blur-md rounded-full text-[11px] font-black text-blue-400 border border-blue-500/10 uppercase tracking-[0.15em]"
                    >
                      {genre.name}
                    </span>
                  ))}
            ...
            <div className="bg-[#151b2b] p-10 rounded-[2.5rem] border border-white/5 space-y-8">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center group">
                  <span className="text-slate-500 font-black uppercase tracking-widest text-[11px] group-hover:text-slate-300 transition-colors">
                    Type
                  </span>
          ...
          <div className="flex-1 space-y-12">
            ...
            <div className="space-y-8">
              <div className="flex items-center justify-between">
                ...
                <span className="bg-[#151b2b] px-5 py-2 rounded-full text-[10px] font-black text-slate-500 uppercase tracking-widest border border-white/5">
                  0 reviews
                </span>
              </div>
